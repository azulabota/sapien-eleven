...
function HeroGraphAnimation() {
  ...
  const clusters = [
    { label: 'Wearables', x: 80, y: 80 },
    { label: 'Nutrition', x: 380, y: 60 },
    { label: 'Workouts', x: 60, y: 240 },
    { label: 'Sleep', x: 400, y: 250 },
    { label: 'Mood', x: 220, y: 40 },
    { label: 'Coach', x: 230, y: 280 },
  ];
    ...
    <div className="relative w-full max-w-2xl mx-auto" style={{ height: 340 }}>
      <svg viewBox="0 0 480 320" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        ...
        {clusters.map((c, i) => (
          <line
            key={`line-${i}`}
            x1={c.x} y1={c.y} x2={hubX} y2={hubY}
            stroke="rgba(160,160,160,0.2)"
            strokeWidth="0.8"
            strokeDasharray="4 3"
            style={{ opacity: phase === 'disconnected' ? 0 : 1, transition: `opacity 0.4s ease ${i * 0.15}s` }}
          />
        ))}
        ...
        {/* Hub outer ring */}
        <circle cx={hubX} cy={hubY} r="36" stroke="rgba(160,160,160,0.1)" strokeWidth="0.6" strokeDasharray="3 4"
          style={{ opacity: phase !== 'disconnected' ? 1 : 0, transition: 'opacity 0.5s ease', animation: phase === 'connected' ? 'spin-slow 25s linear infinite' : 'none' }} />

        {/* Hub inner ring */}
        <circle cx={hubX} cy={hubY} r="24" stroke="rgba(160,160,160,0.2)" strokeWidth="0.8"
        ...
        {clusters.map((c, i) => (
          <g key={`cluster-${i}`}>
            <circle cx={c.x} cy={c.y} r="6" fill="rgba(10,10,10,0.8)" stroke="rgba(160,160,160,0.55)" strokeWidth="1"
              style={{ animation: 'broken-pulse 3s ease-in-out infinite', animationDelay: `${i * 0.4}s` }} />
            <circle cx={c.x} cy={c.y} r="2.5" fill="rgba(200,200,200,0.8)" />
            <circle cx={c.x - 14} cy={c.y + 12} r="2.5" fill="rgba(120,120,120,0.3)" />
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-8">
          ...
          <div className="hero-reveal flex flex-wrap gap-4">
            ...
            <button className="btn-ghost" onClick={() => document.getElementById('data-layer')?.scrollIntoView({ behavior: 'smooth' })}>
              See the Data Layer
            </button>
          </div>
          ...
          {/* Stats */}
          <div
            className="hero-reveal flex flex-wrap gap-8 pt-4"
